...
function plgContentYOOtooltipReplace(&$row, &$matches, $count, $regex, $params) {
	...
	for ($i = 0; $i < $count; $i++) {
		...
		if ($tooltip_text != '') {
			...
			$replace .= "<script type=\"text/javascript\">window.addEvent('domready', function() { new YOOtooltip('". $id ."', '" . plgContentYOOtooltipStripText($tooltip_text) . "', { mode: '" . $mode . "', display: '" . $display . "', width: " . $width . ", style: '" . $style . "', sticky: " . $sticky . " }); });</script>";
		}
