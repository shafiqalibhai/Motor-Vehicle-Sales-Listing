...
?>
<span style='color: black;'>&nbsp;<?php echo ($joomlaWatch->getActualDateTime()); ?></span>

<?php
if (!$joomlaWatch->isModulePublished()) echo("<h4 style='color: red;'>"._JW_VISITS_MODULE_NOT_PUBLISHED."</h4>");


echo ("<br/>");
$today = floor($joomlaWatch->getServerTime()/ 24 / 3600);
$thisWeek = floor($joomlaWatch->getServerTime()/ 24 / 3600 / 7);
if (@ $_GET['day'])
	$day = @ $_GET['day'];
else
	$day = floor($joomlaWatch->getServerTime()/ 24 / 3600);

if (@ $_GET['week'])
	$week = @ $_GET['week'];
else
	$week = floor($joomlaWatch->getServerTime()/ 24 / 3600 / 7);
