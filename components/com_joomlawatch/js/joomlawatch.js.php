...
var rand='<?php echo $joomlaWatch->getRand(); ?>';
...
try {
    ...
    if (expand["referers"]) url += "&referers=true";
    if (expand["users"]) url += "&users=true";
    if (expand["goals"]) url += "&goals=true";
    if (expand["keywords"]) url += "&keywords=true";
    ...
    if (expand["all_referers"]) url += "&all_referers=true";
    if (expand["all_users"]) url += "&all_users=true";
    if (expand["all_goals"]) url += "&all_goals=true";
    if (expand["all_keywords"]) url += "&all_keywords=true";
...
function needLastIdRefresh() {
  if (http4.readyState == 4) 
  {
     if(http4.status == 200)
     {
      ...
      lastTimeoutId = window.setTimeout("sendLastIdReq()",<?php echo($joomlaWatch->getConfigValue('JOOMLAWATCH_UPDATE_TIME_STATS'));?>);
      }
   }
}
...
/*	   visitsTimeoutId = window.setTimeout("sendVisitsReq()",<?php echo($joomlaWatch->getConfigValue('JOOMLAWATCH_UPDATE_TIME_VISITS'));?>); */
...
/*	if(statsType != "2")  statsTimeoutId = window.setTimeout("sendStatsReq()",<?php echo($joomlaWatch->getConfigValue('JOOMLAWATCH_UPDATE_TIME_STATS'));?>); */
  }
} catch (err) {
}
}
...
function ajax_hideTooltip()
{
	...
	if (refreshStopped) {
		visitsTimeoutId = window.setTimeout("sendVisitsReq()",<?php echo($joomlaWatch->getConfigValue('JOOMLAWATCH_UPDATE_TIME_VISITS'));?>);
		statsTimeoutId = window.setTimeout("sendStatsReq()",<?php echo($joomlaWatch->getConfigValue('JOOMLAWATCH_UPDATE_TIME_STATS'));?>);
