...
var FancyUpload = new Class({
	...
	options: {
		...
		removefromqueuetext: 'Remove from queue',
		maxfilesizealert: 'This file exceeds permitted maximum file size!'
	},
	...
	initializeFlash: function() {
		...
		if (this.options.createReplacement) this.options.createReplacement(this.element);
			...
			new Element('input', {
				type: 'button',
				value: this.options.browsefiletext,
				events: {
	...
	onOpen: function(name, size) {
		...
		this.fileList[index].fx = new Element('div', {'class': 'queue-subloader'}).injectInside(
				new Element('div', {'class': 'queue-loader'}).setHTML(this.options.uploadingtext).injectInside(this.fileList[index].element)
			).effect('width', {
				duration: 200,
				wait: false,
				unit: '%',
	...
	onComplete: function(name, size) {
		...
		this.fileList[index].fx.element.setHTML(this.options.completetext);
	...
	addFile: function(name, size) {
		...
		new Element('a', {
			...
			'class': 'input-delete',
			title: this.options.removefromqueuetext,
			events: {
