<?php
//rutube.ru plugin for Seyret component//
/**
* Content code
* @package SEYRET
...
function zippyvideoscomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
    ...
    //http://www.zippyvideos.com/6105413215452076/sunp0411/
    $smallvideocode = str_replace("http://www.zippyvideos.com/","",$vidlink);
    $smallvideocode = str_replace("http://zippyvideos.com/","",$smallvideocode);

	//improved security not to call another site...
	$vidlink = "http://www.zippyvideos.com/".$smallvideocode;


    $videoservertype = "zippyvideos.com";
    ...
    $t = strlen(" @ ZippyVideos.com - Free Video Webhosting");
    $post = strlen($videotitle) - $t;
	$videotitle = substr($videotitle, 0, $post);

	$itemcomment = $videotitle;

    $clipvnurlmatch = "/(.*)";
    $u  = eregi_replace($clipvnurlmatch, '', $smallvideocode );
    $url = "http://www.zippyvideos.com/embed.z?u=".$u;
	$str = jalem_file_get_contents($url);

    $pos = strpos($str, "&imagePrev=", $pos)+11;
	$post = strpos($str, ".jpg", $pos)-$pos+4;
...
function zippyvideoscomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+zippyvideoscom+".$vcode.$usevideoadsystem;
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/zippyvideoscom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/zippyvideoscom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=zippyvideoscomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/zippyvideoscom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/zippyvideoscom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
//----------------------
    ...
    $pos += 11;
    $post = strpos($str, ".jpg", $pos)+4;
    $t = $post - $pos;
    $imagePrev = substr($str, $pos, $t);

    //$embedvideo = "<embed src=\"http://www.motionbox.com/external/hd_player/type=sd,video_uid=$vcode\" type=\"application/x-shockwave-flash\" pluginspage=\"http://www.adobe.com/go/getflashplayer\" width=\"$videowidth\" height=\"$videoheight\" allowFullscreen=\"true\"></embed>";
    $embedvideo = "<embed src=\"http://www.zippyvideos.com/images/eplayer.swf?storageId=$storageId&userName=$userName&videoId=$videoId&srvURL=$srvURL&imagePrev=$imagePrev\" uality=\"high\" bgcolor=\"#000000\" width=\"$videowidth\" height=\"$videoheight\" name=\"puesta2\" align=\"middle\" allowScriptAccess=\"sameDomain\" type=\"application/x-shockwave-flash\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\" />";
    //$embedvideo = zippyvideoscomdownload($vcode); // only for test flv link
//----------------------
...
function zippyvideoscomgeneratevideodownloadlink($vcode, $pro, $dltask){
global $database, $mosConfig_live_site, $mosConfig_absolute_path;

$vtype="zippyvideoscom";
