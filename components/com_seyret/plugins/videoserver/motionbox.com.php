...
function motionboxcomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
    ...
    //http://www.motionbox.com/videos/3c94dab4b8?type=sd
    //http://www.motionbox.com/videos/dc9bdcb356
    //http://www.motionbox.com/video/player/3c94dab4b8
    $clipvnurlmatch = "(.*)videos/";
    $smallvideocode  = eregi_replace($clipvnurlmatch, '', $vidlink );
    $clipvnurlmatch = "(.*)player/";
    $smallvideocode  = eregi_replace($clipvnurlmatch, '', $smallvideocode );

	//improved security not to call another site...
	$vidlink = "http://www.motionbox.com/videos/".$smallvideocode;


    $videoservertype = "motionbox.com";
    ...
    $pos = strpos($str, "<link href=\"", $pos)+12;
	$post = strpos($str, "\" rel=", $pos)-$pos;
...
function motionboxcomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+motionboxcom+".$vcode.$usevideoadsystem;
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/motionboxcom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/motionboxcom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=motionboxcomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/motionboxcom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/motionboxcom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
//----------------------
    //$embedvideo = "<EMBED src=\"http://video.rutube.ru/$vcode\" type=\"application/x-shockwave-flash\" wmode=\"window\" width=\"$videowidth\" height=\"$videoheight\" allowFullScreen=\"true\" flashVars=\"uid=2655013\"></EMBED>";
    ...
    //$embedvideo = motionboxcomdownload($vcode); // only for test flv link
//----------------------
...
function motionboxcomgeneratevideodownloadlink($vcode, $pro, $dltask){
