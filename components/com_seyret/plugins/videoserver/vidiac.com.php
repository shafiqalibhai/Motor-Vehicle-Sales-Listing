<?php
//Vidiac plugin for Seyret component//
/**
* Content code
* @package SEYRET
...
function vidiaccomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
	...
	$vidlink=jalemurldecode($vidlink);	
		...
		$videoservertype="vidiac.com";
		$smallvideocode=str_replace("http://www.vidiac.com/video/","",$vidlink);
		//http://www.vidiac.com/video/2471c448-d1a4-4d7a-afbc-9b2e3b8aa445.htm
		//improved security not to call another site...
		$vidlink="http://www.vidiac.com/video/".$smallvideocode;	
	
} else if ($reqtype=="refresh") {
	if ($vidlink==""){
		$vidlink="http://www.vidiac.com/video/".$existingcode;	
	}	
	
}
		$smallvideocode=str_replace(".htm","",$smallvideocode);
		
		$infolink="http://www.vidiac.com/playlistbuilder/buildflash2.ashx?fileid=".$smallvideocode;
		$str=jalem_file_get_contents($infolink);
		

		
		$pos = strpos($str, "description=")+13;
		$post=strpos($str, "url=")-$pos-2;		
...
function vidiaccomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+vidiaccom+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/vidiaccom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/vidiaccom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=vidiaccomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/vidiaccom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/vidiaccom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
$embedvideo="<embed src=\"http://www.vidiac.com/vidiac.swf\" FlashVars=\"video=".$vcode."\" quality=\"high\" bgcolor=\"#ffffff\" width=\"".$videowidth."\" height=\"".$videoheight."\" name=\"ePlayer\" align=\"middle\" allowScriptAccess=\"sameDomain\" type=\"application/x-shockwave-flash\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\"></embed>";
}
...
function vidiaccomgeneratevideodownloadlink($vcode, $pro, $dltask){
