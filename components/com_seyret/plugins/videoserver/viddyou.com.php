<?php
//viddyou.com plugin for Seyret component//
/**
* Content code
* @package SEYRET
...
function viddyoucomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
    ...
    //http://www.viddyou.com/viddstream?videoid=27331
    $clipvnurlmatch = "(.*)videoid=";
    $smallvideocode  = eregi_replace($clipvnurlmatch, "", $vidlink );

    //improved security not to call another site...
	$vidlink = "http://www.viddyou.com/viddstream?videoid=".$smallvideocode;

    $videoservertype = "viddyou.com";
    ...
    $videotitle = substr($videotitle, 13, strlen($videotitle)-13); //Viddyou.com:

    $pos = strpos($str, "name=\"description\" content=\"", $post)+28;
	$post = strpos($str, "\"", $pos);
    $l = $post-$pos;
	$itemcomment = substr($str, $pos, $l);
...
function viddyoucomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+viddyoucom+".$vcode.$usevideoadsystem;
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/viddyoucom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/viddyoucom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=viddyoucomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/viddyoucom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/viddyoucom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
//----------------------
    $embedvideo = "<embed src=\"http://www.viddyou.com/get/v2_full/$vcode.swf\" type=\"application/x-shockwave-flash\" allowfullscreen=\"true\" width=\"$videowidth\" height=\"$videoheight\"></embed>";
    //$embedvideo = "<embed allowFullScreen=\"true\" src=\"http://crackle.com/p/Sketch_Comedy_Collection/Role_Models.swf\" width=\"$videowidth\" height=\"$videoheight\" quality=\"high\" scale=\"noScale\" FlashVars=\"id=$vcode&amp;ml=o%3D12%26fpl%3D296943%26fx%3D\" wmode=\"window\" pluginspage=\"http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash\" type=\"application/x-shockwave-flash\"></embed>";
    //$embedvideo = viddyoucomdownload($vcode); // only for test flv link
//----------------------
...
function viddyoucomgeneratevideodownloadlink($vcode, $pro, $dltask){
