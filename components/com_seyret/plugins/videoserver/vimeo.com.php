...
$downloadcachingtimeout="11";

function vimeocomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
		...
		$videoservertype="vimeo.com";

		$infolink="http://www.vimeo.com/moogaloop/load/clip:".$smallvideocode."/embed";
		

		$str=jalem_file_get_contents($infolink);
		
		$pos = strpos($str, "<caption>")+9;
		$post=strpos($str, "</caption>")-$pos;		
		$videotitle=substr($str,$pos,$post);
			

		$pos = strpos($str, "<thumbnail>")+11;
		$post=strpos($str, "</thumbnail>")-$pos;		
...
function vimeocomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+vimeocom+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/vimeocom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/vimeocom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=vimeocomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/vimeocom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/vimeocom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
$embedvideo="<embed src=\"http://www.vimeo.com/moogaloop.swf?clip_id=".$vcode."\" quality=\"best\" scale=\"exactfit\" width=\"".$videowidth."\" height=\"".$videoheight."\" type=\"application/x-shockwave-flash\"></embed>";
}
...
function vimeocomgeneratevideodownloadlink($vcode, $pro, $dltask){
