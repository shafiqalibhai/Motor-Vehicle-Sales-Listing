<?php
//crackle.com plugin for Seyret component//
/**
* Content code
* @package SEYRET
...
function cracklecomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
    ...
    //http://crackle.com/c/Sketch_Comedy_Collection/Role_Models/2005534#id=1468142&ml=o%3D12%26fpl%3D296943%26fx%3D

    $t = strpos($vidlink, "#");
    if ($t > 0){
        $vidlink = substr($vidlink, 0, $t);
    }

    $pieces = explode("/", $vidlink);
    $smallvideocode = $pieces[6]; // piece1

    //improved security not to call another site...
	//$vidlink = "http://www.vsocial.com/video/?d=".$smallvideocode;
    //$vidlink = "http://crackle.com/c/Sketch_Comedy_Collection/Role_Models/2005534";

    $videoservertype = "crackle.com";

	$str = jalem_file_get_contents($vidlink);
    //echo $str; exit;
    $pos = strpos($str, "name=\"title\" content=\"", $post)+22;
	$post = strpos($str, "\" />", $pos);
    $l = $post-$pos;
    ...
    $picturelink = substr($picturelink, 18, strlen($picturelink)-18);//http://crackle.com
...
function cracklecomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+cracklecom+".$vcode.$usevideoadsystem;
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/cracklecom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/cracklecom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=cracklecomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/cracklecom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/cracklecom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
//----------------------
    ...
    //$embedvideo = cracklecomdownload($vcode); // only for test flv link
//----------------------
...
function cracklecomgeneratevideodownloadlink($vcode, $pro, $dltask){
