<?php
//video Videos plugin for Seyret component//
/**
* Content code
* @package SEYRET
...
function videomsncomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
		...
		$videoservertype="video.msn.com";
		$pos = strpos($vidlink, "vid=")+4;
		$smallvideocode=substr($vidlink,$pos,100);
	
		

		//improved security not to call another site...
		$vidlink="http://video.msn.com/video.aspx?vid=".$smallvideocode;
	
} else if ($reqtype=="refresh") {

	
}		
		//http://video.msn.com/video.aspx?mkt=en-US&brand=&vid=e0aef7de-7450-4426-9703-9348e6a35804

		$picturelinkdet="http://edge2.catalog.video.msn.com/videoByUuids.aspx?uuids=".$smallvideocode."&rct=1,3,5,6";
		

		
		$pictdet=jalem_file_get_contents($picturelinkdet);
		
		$pos = strpos($pictdet, "<file formatCode=\"2001\"");
		$post=strpos($pictdet, ".jpg")-$pos;		
		$picturelink=substr($pictdet,$pos,$post+4);
		$picturelink=strip_tags($picturelink);		

		
		
		$pos = strpos($pictdet, "<title>")+7;
		$post=strpos($pictdet, "</title>")-$pos;				
		$videotitle=substr($pictdet,$pos,$post);		
		$videotitle=strip_tags($videotitle);

		$pos = strpos($pictdet, "<description>")+13;
		$post=strpos($pictdet, "</title>")-$pos;				
		$itemcomment=substr($pictdet,$pos,$post);		
		$itemcomment=strip_tags($videotitle);		
...
function videomsncomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+videomsncom+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/videomsncom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/videomsncom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=videomsncomgeneratevideodownloadlink($vcode, $pro, "embed");
			
			$dwnlink=str_replace("&amp;","&",$dwnlink);
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/videomsncom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/videomsncom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
$embedvideo="<embed src=\"http://video.msn.com/flash/soapbox1_1.swf\" quality=\"high\" width=\"".$videowidth."\" height=\"".$videoheight."\" wmode=\"transparent\" type=\"application/x-shockwave-flash\" pluginspage=\"http://macromedia.com/go/getflashplayer\" flashvars=\"c=v&v=".$vcode."\" ></embed>";
}
return $embedvideo;
...
function videomsncomgeneratevideodownloadlink($vcode, $pro, $dltask){
