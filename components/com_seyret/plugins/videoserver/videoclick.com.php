<?php
//videoclick.com plugin for Seyret component//
...
function videoclickcomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
    ...
    //http://www.videoclick.com/video-/videos_kitesurfpartie_a_lentree_du_port_de_st_marguerite_costabrava.html
    //http://www.videoclick.com/video-click_sport/videos_some_fun_in_the_waves.html
	$smallvideocode = str_replace("http://www.videoclick.com/", "", $vidlink);
	$smallvideocode = str_replace("http://videoclick.com/", "", $smallvideocode);
	//improved security not to call another site...
	$vidlink = "http://www.videoclick.com/".$smallvideocode;


    $videoservertype = "videoclick.com";
    ...
    $videotitle = substr($videotitle, 0, strlen($videotitle)-13);// | Videoclick
    $videotitle = trim($videotitle);

    $pos = strpos($str, "class=\"teaser\">")+15;
	$post = strpos($str, "</div>", $pos)-$pos;
    $itemcomment = substr($str, $pos, $post);
    $itemcomment = trim($itemcomment);
    ...
    //http://player2.rpe.ipercast.net/player/player.js.php?media=16598&width=512&height=384&portal=213&medianame=Rodney Mullen 1.wmv&theme=1&soustheme=1&z-index=1
    //http://player2.rpe.ipercast.net/player/videoclick.js.php?media=00-48-852-48852_53.flv&width=512&height=384
    $ok = strpos($url, ".wmv&");
    if ($ok > 1){
    	...
    	$pos = $post+18;//flashiddiffuseur=
        $post = strpos($str, "\"", $pos)-$pos;
        $iddiffuseur = substr($str, $pos, $post);
    }

    //http://video.videoclick.com/flash_videoclick.php?idmedia=00-48-918-48918_swing_basile.flv&iddiffuseur=213
...
function videoclickcomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+videoclickcom+".$vcode.$usevideoadsystem;
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/videoclickcom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/videoclickcom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=videoclickcomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/videoclickcom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/videoclickcom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
//----------------------
    ...
    //$embedvideo = $idmedia;
    //<embed src="http://video.videoclick.com/videoclick_tv.swf?flashidmedia=00-48-918-48918_swing_basile.flv&flashiddiffuseur=213" quality="high" wmode="transparent" width="512" height="384" name="playerUniversel" align="middle" allowFullScreen="true" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
    ...
    $embedvideo = "<embed src=\"http://video.videoclick.com/videoclick_tv.swf?flashidmedia=$idmedia&flashiddiffuseur=$iddiffuseur\" quality=\"high\" wmode=\"transparent\" width=\"$videowidth\" height=\"$videoheight\" name=\"playerUniversel\" align=\"middle\" allowFullScreen=\"true\" type=\"application/x-shockwave-flash\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\"></embed>";
    //$embedvideo = videoclickcomdownload($vcode); // only for test flv link
//----------------------
...
function videoclickcomgeneratevideodownloadlink($vcode, $pro, $dltask){
