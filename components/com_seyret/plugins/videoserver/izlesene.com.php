...
$downloadcachingtimeout="1000";

function izlesenecomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
		...
		$smallvideocode=str_replace("http://www.izlesene.com/video/","",$vidlink);
		//improved security not to call another site...
		$vidlink="http://www.izlesene.com/video/".$smallvideocode;
	
} else if ($reqtype=="refresh") {
	if ($vidlink==""){
		$vidlink="http://www.izlesene.com/video/".$existingcode;
	}	
		...
		$pos = strpos($smallvideocode, "/");
		if ($pos>0) {
			$smallvideocode2=substr($smallvideocode, $pos + 1, 100);
		}		

		
		$videoservertype="izlesene.com";
...
function izlesenecomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
	...
	$videoheight=$popupvideoheight;
		...
		$pos = strpos($vcode, "/")+1;
		$strip1=substr($vcode,$pos,500);
		$post=strpos($strip1, "/");		
		$embedcode=substr($strip1,0,$post);
		$shortcode=substr($embedcode,0,2);
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+izlesenecom+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/izlesenecom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/izlesenecom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=izlesenecomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/izlesenecom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/izlesenecom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
$embedvideo="<object width=\"".$videowidth."\" height=\"".$videoheight."\"><param name=\"movie\" value=\"http://www.izlesene.com/player2.swf?izle=/data/videos/".$shortcode."/".$embedcode.".flv\" /><param name=\"quality\" value=\"high\" /><param name=\"bgcolor\" value=\"#ffffff\" /><embed src=\"http://www.izlesene.com/player2.swf?izle=/data/videos/".$shortcode."/".$embedcode.".flv\" quality=\"high\" bgcolor=\"#ffffff\" width=\"".$videowidth."\" height=\"".$videoheight."\" type=\"application/x-shockwave-flash\" /></object>";
}
...
function izlesenecomgeneratevideodownloadlink($vcode, $pro, $dltask){
