...
function stupidvideoscomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
		...
		$videoservertype="stupidvideos.com";
		$smallvideocode=str_replace("http://www.stupidvideos.com/","",$vidlink);
		$smallvideocode=str_replace("http://stupidvideos.com/","",$smallvideocode);
		//improved security not to call another site...
		$vidlink="http://www.stupidvideos.com/".$smallvideocode;
		...
		$videotitle=str_replace(" and other videos on StupidVideos.com","",$videotitle);

		
		$pos = strpos($str, "<META name=\"description\" content=\"")+34;
		$post=strpos($str, "\"><meta name=\"title\"")-$pos;
		$itemcomment=substr($str,$pos,$post);
	    $itemcomment=strip_tags($itemcomment);


		$pos = strpos($str, "<link rel=\"videothumbnail\" href=\"")+33;
		$post=strpos($str, "\" type=\"image/jpeg\"/>")-$pos;
		$picturelink=substr($str,$pos,$post);

		
		$pos = strpos($str, "var videoID =")+13;
		$post=strpos($str, "var currentVideoID")-$pos;
...
function stupidvideoscomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+stupidvideoscom+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/stupidvideoscom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/stupidvideoscom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=stupidvideoscomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/stupidvideoscom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/stupidvideoscom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
$embedvideo="<embed src=\"".$mosConfig_live_site."/components/com_seyret/localplayer/seyretplayer.swf\" width=\"".$videowidth."\" height=\"".$videoheight."\" bgcolor=\"#FFFFFF\" type=\"application/x-shockwave-flash\" flashvars=\"file=".$vlink."&image=".$vthumb."&showdigits=false&autostart=false&logo=".$mosConfig_live_site."/components/com_seyret/localplayer/logo.png&type=flv&repeat=false&backcolor=0x000000&frontcolor=0xCCCCCC&usefullscreen=false\" /></embed>";
}
...
function stupidvideoscomgeneratevideodownloadlink($vcode, $pro, $dltask){
global $database, $mosConfig_live_site, $mosConfig_absolute_path;

$vtype="stupidvideoscom";
