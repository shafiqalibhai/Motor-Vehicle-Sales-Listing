<?php
//Metacafe plugin for Seyret component//
/**
* Content code
* @package SEYRET
...
function putfilecomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
		...
		$smallvideocode=str_replace("http://media.putfile.com/","",$vidlink);
		
		//improved security not to call another site...
		$vidlink="http://media.putfile.com/".$smallvideocode;	
		...
		$videoservertype="putfile.com";
		
		$str=jalem_file_get_contents($vidlink);
		$pos = strpos($str, "<TITLE>")+7;
		$post=strpos($str, "</TITLE>")-$pos;		
		$videotitle=substr($str,$pos,$post);
		$videotitle=str_replace("- Putfile.com","",$videotitle);


		$picturelink="http://flv.putfile.com/images/".$smallvideocode.".jpg";	
...
function putfilecomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+putfilecom+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/putfilecom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/putfilecom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=putfilecomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/putfilecom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/putfilecom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
<meta rel=\"type\">video/flv</meta>
	...
	if (isset($userecommendations)) {
		if ($userecommendations=="1"){
			$recommendationsfile=getrecommendations($vcode, $catid);
			$recommendations="&recommendations=".$recommendationsfile;
		}
	} else {
		$recommendations="";
	}
...
$embedvideo="<object type=\"application/x-shockwave-flash\" data=\"http://feat.putfile.com/flow/putfile.swf?videoFile=".$vcode."\" height=\"".$videoheight."\" width=\"".$videowidth."\" align=\"middle\"> <param name=\"movie\" value=\"http://feat.putfile.com/flow/putfile.swf?videoFile=".$vcode."\" /> <param name=\"quality\" value=\"high\" /></object>";
}
...
function putfilecomgeneratevideodownloadlink($vcode, $pro, $dltask){
