...
function myspacecomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
		...
		$vidlink=strtolower($vidlink);	
		$smallvideocode=str_replace("http://vids.myspace.com/index.cfm?fuseaction=vids.individual&videoid=","",$vidlink);

		//improved security not to call another site...
		$vidlink="http://vids.myspace.com/index.cfm?fuseaction=vids.individual&videoid=".$smallvideocode;
	
} else if ($reqtype=="refresh") {
	if ($vidlink==""){
	$vidlink="http://vids.myspace.com/index.cfm?fuseaction=vids.individual&videoid=".$existingcode;
	}	
		...
		$videoservertype="myspace.com";

		$infolink="http://mediaservices.myspace.com/services/rss.ashx?type=video&mediaID=".$smallvideocode;
		

		$str=jalem_file_get_contents($infolink);
		
		$pos = strpos($str, "<item>")+7;
		$str=substr($str,$pos,5000);
		...
		$pos = strpos($str, "<media:thumbnail")+22;
		$post=strpos($str, "<media:content url=")-$pos;		
		$picturelink=substr($str,$pos,$post);
		$picturelink=str_replace("/>","",$picturelink);

		$desc=jalem_file_get_contents($vidlink);
		$pos = strpos($desc, "<b>Description:</b>")+19;
		$itemcommentstrip1=substr($desc,$pos,1000);

		$pos = strpos($desc, "<hr size=\"1\"");
		$itemcommentstrip2=substr($itemcommentstrip1,0,$pos);
		$itemcomment=strip_tags($itemcommentstrip2);
...
function myspacecomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+myspacecom+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/myspacecom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/myspacecom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=myspacecomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/myspacecom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/myspacecom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
$embedvideo="<embed src=\"http://lads.myspace.com/videos/vplayer.swf\" flashvars=\"m=".$vcode."&type=video\" type=\"application/x-shockwave-flash\" width=\"".$videowidth."\" height=\"".$videoheight."\"></embed>";
}
...
function myspacecomgeneratevideodownloadlink($vcode, $pro, $dltask){
