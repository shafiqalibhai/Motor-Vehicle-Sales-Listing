<?php
//Glumbert Videos plugin for Seyret component//
/**
* Content code
* @package SEYRET
...
function glumbertcomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
		...
		$smallvideocode=str_replace("http://www.glumbert.com/media/","",$vidlink);
		//improved security not to call another site...
		$vidlink="http://www.glumbert.com/media/".$smallvideocode;
	
} else if ($reqtype=="refresh") {
	if ($vidlink==""){
		$vidlink="http://www.glumbert.com/media/".$existingcode;//trytoguess
	}	
		...
		//$itemcomment=substr($trim1,$posp,300);		


		
		$picturelink="http://cdn.glumbert.com/graphics/media/k/lg-".$smallvideocode.".jpg";
...
function glumbertcomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+glumbertcom+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/glumbertcom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/glumbertcom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=glumbertcomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/glumbertcom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/glumbertcom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
$embedvideo="<object width=\"".$videowidth."\" height=\"".$videoheight."\"><param name=\"movie\" value=\"http://www.glumbert.com/embed/".$vcode."\"></param><param name=\"wmode\" value=\"transparent\"></param><embed src=\"http://www.glumbert.com/embed/".$vcode."\" type=\"application/x-shockwave-flash\" wmode=\"transparent\" width=\"".$videowidth."\" height=\"".$videoheight."\"></embed></object>";
}
...
function glumbertcomgeneratevideodownloadlink($vcode, $pro, $dltask){
