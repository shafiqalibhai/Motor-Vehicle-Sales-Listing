...
function abrutiscomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
    ...
    //http://www.abrutis.com/video-chien+jedi-14981.html
	$smallvideocode = str_replace("http://www.abrutis.com/", "", $vidlink);
	$smallvideocode = str_replace("http://abrutis.com/", "", $smallvideocode);
    $smallvideocode = str_replace(".html", "", $smallvideocode);
	//improved security not to call another site...
	$vidlink = "http://www.abrutis.com/".$smallvideocode.".html";


    $videoservertype = "abrutis.com";
    ...
    $pos = strlen("Abrutis.com");
    $videotitle = substr($videotitle, $pos, strlen($videotitle)-$pos);
    $videotitle = trim($videotitle);

    $pos = strpos($str, "name=\"description\"");
	$post = strpos($str, "content=\"", $pos)+8;
    $pos = strpos($str, "\" />", $post)-$post;
    $itemcomment = substr($str, $post, $pos);
    $itemcomment = trim($itemcomment);

	$pos = strpos($str, "&file=");
	$post = strpos($str, ".com/", $pos)+5;
    $pos = strpos($str, ".flv", $post)-$post;
    $url = substr($str, $post, $pos);

    $picturelink = "http://thumbs.diff.abrutis.com/$url/thumb/3.jpg";
...
function abrutiscomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+abrutiscom+".$vcode.$usevideoadsystem;
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/abrutiscom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/abrutiscom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=abrutiscomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/abrutiscom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/abrutiscom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
//----------------------
    ...
    $url = substr($str, $pos, $post);

    $embedvideo="<embed src=\"http://www.abrutis.com/jw_player/player.swf\" allowfullscreen=\"true\" height=\"".$videoheight."\" width=\"".$videowidth."\" type=\"application/x-shockwave-flash\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\" flashvars=\"file=".$url."&bufferlength=3&volume=50&autostart=false&fullscreen=true\"/>";
...
function abrutiscomgeneratevideodownloadlink($vcode, $pro, $dltask){
