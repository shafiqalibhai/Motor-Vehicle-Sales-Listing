<?php
//Milliyet Videos plugin for Seyret component//
/**
* Content code
* @package SEYRET
...
function videomilliyetcomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
		...
		$videoservertype="video.milliyet.com";
		$smallvideocode=str_replace("http://video.milliyet.com.tr/videolar.asp?","",$vidlink);
		
		//security
		$vidlink="http://video.milliyet.com.tr/videolar.asp?".$smallvideocode;
		...
		$pos = strpos($smallvideocode, "&id=")+1;
		$smallvideocode=substr($smallvideocode,$pos,100);
		
		$post = strpos($smallvideocode, "&get=");
		if ($post<>0){
		...
		$pos = strpos($str, "<h1><a href=\"\" class=\"white_b\">")+31;
		$post=strpos($str, "<h3><a href=\"\" class=\"gray_1\">")-$pos;		
		$videotitle=substr($str,$pos,$post);
		$videotitle=str_replace("</a></h1>","",$videotitle);

		$pos = strpos($str, "<h3><a href=\"\" class=\"gray_1\">")+30;
		$post=strpos($str, "<div class=\"line\"></div>")-$pos;		
		$itemcomment=substr($str,$pos,$post);	
		$itemcomment=strip_tags($itemcomment);
		//ok
		$thumbprefix="http://video.milliyet.com.tr/web/images/video/80/";
		
		$pos = strpos($smallvideocode, "&tarih=");
		$videoid=substr($smallvideocode,0,$pos);
		$videoid=str_replace("id=","",$videoid);
		
		$cutit="id=".$videoid."&tarih=";
		$tarih=str_replace($cutit,"",$smallvideocode);
		
		$picturelink=$thumbprefix.$tarih."/".$videoid.".jpg";
...
function videomilliyetcomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+videomilliyetcom+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/videomilliyetcom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/videomilliyetcom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=videomilliyetcomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/videomilliyetcom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/videomilliyetcom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
$embedvideo="<object width=".$videowidth." height=".$videoheight."><param name=\"movie\" value=\"http://video.milliyet.com.tr/m.swf?".$vcode."\"></param><param name=\"wmode\" value=\"transparent\"></param><embed src=\"http://video.milliyet.com.tr/m.swf?".$vcode."\" type=\"application/x-shockwave-flash\" wmode=\"transparent\" width=".$videowidth." height=".$videoheight."></embed></object>";
}
...
function videomilliyetcomgeneratevideodownloadlink($vcode, $pro, $dltask){
global $database, $mosConfig_live_site, $mosConfig_absolute_path;

$vtype="videomilliyetcom";
