...
if ($reqtype=="new") {
	...
	if ($vidlink==""){
		...
		$post=strpos($str, "<link rel=\"shortcut icon")-$pos;		
		...
		$post=strpos($str, "<meta name=\"title\" content=\"")-$pos;		
		$picturelink=substr($str,$pos,$post);	
		$picturelink=str_replace("/>","",$picturelink);	
		$picturelink=str_replace("\"","",$picturelink);	
		$picturelink=trim($picturelink);
		$pos = strpos($picturelink, ".jpg")+4;
		$picturelink=substr($picturelink,0,$pos);
...
function metacafeembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+metacafe+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/metacafe/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/metacafe/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=metacafegeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/metacafe";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/metacafe/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
$embedvideo="<embed src=\"http://www.metacafe.com/fplayer/".$vcode."\" width=\"".$videowidth."\" height=\"".$videoheight."\" wmode=\"transparent\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\" type=\"application/x-shockwave-flash\"></embed>";
}
...
function metacafegeneratevideodownloadlink($vcode, $pro, $dltask){
