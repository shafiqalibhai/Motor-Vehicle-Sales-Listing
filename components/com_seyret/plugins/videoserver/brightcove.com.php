...
//**************************************type3
	...
	if ($vidlink==""){
		...
		$pos = strpos($str, "<div class=\"titleDesc\">")+23;
		$post=strpos($str, "<div class=\"titleGrayArea\">")-$pos;
...
$req = "\x00\x00\x00\x00\x00\x01\x002com.brightcove.catalog.CatalogFacade.findVideoById\x00\x02/1\x00\x00\x00\x14\x0a\x00\x00\x00\x03\x02\x00" . chr(strlen($smallvideocode)) . $smallvideocode . "\x01\x01\x05";
            $out = "POST /services/amfgateway HTTP/1.1\r\n" .
                   "Host: services.brightcove.tv\r\n" .
                   "User-Agent: Mozilla/5.0 (Windows; U; Windows NT 5.1; zh-CN; rv:1.8.1.1) Gecko/20061204 Firefox/2.0.0.1\r\n" .
                   "Referer: http://admin.brightcove.tv/destination/player/player.swf?allowFullScreen=true&initVideoId={$smallvideocode}&servicesURL=http://services.brightcove.tv/services&viewerSecureGatewayURL=https://services.brightcove.tv/services/amfgateway&cdnURL=http://admin.brightcove.tv&autoStart=false\r\n" .
                   "Content-type: application/x-amf\r\n" .
                   "Content-length: " . strlen($req) . "\r\n" .
                   "Connection: close\r\n\r\n" .
                   $req;
            $fp = fsockopen(gethostbyname("services.brightcove.tv"), 80, $errno, $errstr, 5);
            fwrite($fp, $out);
            $in = "";
            while (!feof($fp)) {
                $s = fgets($fp, 1024);
...
$embedvideo="<embed src=\"http://admin.brightcove.tv/destination/player/player.swf\" bgcolor=\"#FFFFFF\" flashVars=\"allowFullScreen=true&initVideoId=".$vcode."&servicesURL=http://www.brightcove.tv&viewerSecureGatewayURL=https://www.brightcove.tv&cdnURL=http://admin.brightcove.tv&autoStart=false\" base=\"http://admin.brightcove.tv\" name=\"bcPlayer\" width=\"".$videowidth."\" height=\"".$videoheight."\" allowFullScreen=\"true\" allowScriptAccess=\"always\" seamlesstabbing=\"false\" type=\"application/x-shockwave-flash\" swLiveConnect=\"true\" pluginspage=\"http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash\"></embed>";
}
