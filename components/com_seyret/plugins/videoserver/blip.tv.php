<?php
//blip.tv plugin for Seyret component//
/**
* Content code
* @package SEYRET
...
function bliptvgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
	...
	if ($vidlink==""){
		...
		$videoservertype="blip.tv";
		
		$str=jalem_file_get_contents($vidlink);
		$pos = strpos($str, "setPrimaryMediaUrl")+19;
		$post=strpos($str, "setPermalinkUrl")-$pos;		
		$smallvideocodestrip1=substr($str,$pos,$post);
		$pos = strpos($smallvideocodestrip1, "http");
		$post=strpos($smallvideocodestrip1, ".flv")+4-$pos;
		$smallvideocode=substr($smallvideocodestrip1,$pos,$post);		
		...
		$post=strpos($str, "<meta http-equiv=\"keywords\"")-$pos;		
		...
		$pos = strpos($str, "player.setThumbnail");
		$picturelink=substr($str,$pos,300);
		$picturelink = eregi_replace("(.*)http","http", $picturelink);
		$picturelink = eregi_replace("jpg(.*)","jpg", $picturelink);
...
function bliptvembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+bliptv+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/bliptv/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/bliptv/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=bliptvgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/bliptv";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/bliptv/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
function bliptvgeneratevideodownloadlink($vcode, $pro, $dltask){
