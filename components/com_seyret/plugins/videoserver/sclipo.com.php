<?php
//sclipo.com plugin for Seyret component//
/**
* Content code
* @package SEYRET
...
function sclipocomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
	...
	$smallvideocode=str_replace("http://www.sclipo.com/controller.php?p_action=view_detail&video_id=","",$vidlink);
	$smallvideocode=str_replace("http://sclipo.com/controller.php?p_action=view_detail&video_id=","",$smallvideocode);
	//improved security not to call another site...
	$vidlink="http://www.sclipo.com/controller.php?p_action=view_detail&video_id=".$smallvideocode;
	
} else if ($reqtype=="refresh") {
	if ($vidlink==""){
		$vidlink="http://www.sclipo.com/controller.php?p_action=view_detail&video_id=".$existingcode;//trytoguess
	}	
	
}

	
		$videoservertype="sclipo.com";
		
		$str=jalem_file_get_contents($vidlink);		
		$pos = strpos($str, "<meta name=\"Title\" content=\"")+36;
		$post=strpos($str, "<meta name=\"description\" content=\"")-$pos;		
		...
		$req = "abecesfcna=25&video%5Fid=$smallvideocode&p%5Faction=get%5Fvideostream";
		$out = "POST /controller.php HTTP/1.1\r\n" .
				...
				"Referer: http://sclipo.com/outer_flvplayer_new.swf?file=$smallvideocode\r\n" .
		...
		$pos = strpos($strbuffer, "image_path=")+11;
		$post=strpos($strbuffer, ".jpg")-$pos;		
		$picturelink=substr($strbuffer,$pos,$post+4);
...
function sclipocomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+sclipocom+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/sclipocom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/sclipocom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=sclipocomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/sclipocom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/sclipocom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
$embedvideo="<object width=\"".$videowidth."\" height=\"".$videoheight."\"><param name=\"movie\" value=\"http://sclipo.com/outer_flvplayer_new.swf?file=".$vcode."\"><param name=\"wmode\" value=\"transparent\"><embed src=\"http://sclipo.com/outer_flvplayer_new.swf?file=".$vcode."\" type=\"application/x-shockwave-flash\" wmode=\"transparent\" width=\"".$videowidth."\" height=\"".$videoheight."\"></embed></object>";
...
function sclipocomgeneratevideodownloadlink($vcode, $pro, $dltask){
