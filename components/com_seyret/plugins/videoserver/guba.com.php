<?php
//Guba.com plugin for Seyret component//
/**
* Content code
* @package SEYRET
...
function gubacomgetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
		...
		$smallvideocode=str_replace("http://www.guba.com/watch/","",$vidlink);
		...
		$vidlink="http://www.guba.com/watch/".$smallvideocode;
	
} else if ($reqtype=="refresh") {
	if ($vidlink==""){
		$vidlink="http://www.guba.com/watch/".$existingcode;
	}	
		...
		$videoservertype="guba.com";
		

		$str=jalem_file_get_contents($vidlink);
		$pos = strpos($str, "var theName")+13;
		$post=strpos($str, "function rolloverImage(whichId)")-$pos;		
		$videotitle=substr($str,$pos,$post);
		$videotitle=str_replace("\";","",$videotitle);

		
		
		$pos = strpos($str, "Tags:")+5;
		$post=strpos($str, "below_video\"")-$pos;		
		$itemcomment=substr($str,$pos,$post);	
		$itemcomment=strip_tags($itemcomment);
		
		$picturelink="http://img.guba.com/public/video/f/58/".$smallvideocode.".jpg";	
...
function gubacomembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+gubacom+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/gubacom/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/gubacom/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=gubacomgeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/gubacom";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/gubacom/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
if ($pro<>"1" OR $unexpectederror=="1"){
	$embedvideo="<embed src=\"".$mosConfig_live_site."/components/com_seyret/localplayer/seyretplayer.swf\" width=\"".$videowidth."\" height=\"".$videoheight."\" bgcolor=\"#FFFFFF\" type=\"application/x-shockwave-flash\" flashvars=\"file=http://free.guba.com/uploaditem/".$vcode."/flash.flv&image=".$vthumb."&showdigits=false&autostart=false&logo=".$mosConfig_live_site."/components/com_seyret/localplayer/logo.png&type=flv&repeat=false&backcolor=0x000000&frontcolor=0xCCCCCC&usefullscreen=false\" /></embed>";
}
return $embedvideo;
...
function gubacomgeneratevideodownloadlink($vcode, $pro, $dltask){
