...
function myvideodegetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
		...
		$smallvideocode=str_replace("http://www.myvideo.de/watch/","",$vidlink);
		$smallvideocode=str_replace("http://myvideo.de/watch/","",$smallvideocode);

		
		//improved security not to call another site...
		$vidlink="http://www.myvideo.de/watch/".$smallvideocode;
		$videoservertype="myvideo.de";

		$infolink="http://www.myvideo.de/movie/".$smallvideocode;
	...
	exit;
		...
		$newurl=substr($newurl,$pos,$post);

		$stripnewurl=str_replace($smallvideocode,"",$newurl);

		$picturelink=$stripnewurl."/thumbs/".$smallvideocode."_1.jpg";
		...
		$videotitle=str_replace("MyVideo - ","",$videotitle);
...
function myvideodeembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+myvideode+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/myvideode/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/myvideode/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=myvideodegeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/myvideode";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/myvideode/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
$embedvideo="<object classid=\"clsid:d27cdb6e-ae6d-11cf-96b8-444553540000\" width=\"".$videowidth."\" height=\"".$videoheight."\"><param name=\"movie\" value=\"http://www.myvideo.de/movie/".$vcode."\"></param><embed src=\"http://www.myvideo.de/movie/".$vcode."\" width=\"".$videowidth."\" height=\"".$videoheight."\" type=\"application/x-shockwave-flash\"></embed></object>";
}
...
function myvideodegeneratevideodownloadlink($vcode, $pro, $dltask){
