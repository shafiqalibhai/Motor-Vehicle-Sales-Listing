...
function clipfishdegetvideodetails($vidlink, $existingcode,$categorylist, $reqtype){
...
if ($reqtype=="new") {
		...
		$smallvideocode=str_replace("http://www.clipfish.de/player.php?videoid=","",$vidlink);
		$smallvideocode=str_replace("http://clipfish.de/player.php?videoid=","",$smallvideocode);
		
		$pos = strpos($smallvideocode, "=");
		if ($pos>0) $smallvideocode=substr($smallvideocode,0,$pos);
		$pos = strpos($smallvideocode, "&");
		if ($pos>0) $smallvideocode=substr($smallvideocode,0,$pos);
			
		$vidlink="http://www.clipfish.de/player.php?videoid=".$smallvideocode;
		
} else if ($reqtype=="refresh") {
	if ($vidlink==""){
		$vidlink="http://www.clipfish.de/player.php?videoid=".$existingcode;
	}	
		...
		$infolink="http://www.clipfish.de/video_n.php?videoid=".$smallvideocode."&p=0";
		$str=jalem_file_get_contents($infolink);
		
		$pos = strpos($str, "http");
		$post=strpos($str, ".flv")-$pos;
		$picturelink=substr($str,$pos,$post);
		$picturelink=$picturelink."_120x90.jpg";
...
function clipfishdeembed($vcode, $vthumb, $downloadcachingnotimeout, $downloadcachingtimeout, $pro, $catid, $setwidth=null, $setheight=null){
...
if ($pro=="1"){
	...
	$cachefile=$mosConfig_live_site."+clipfishde+".$vcode.$usevideoadsystem;	
	$md5cachefile=md5($cachefile).".xml";
	$fullmd5cachefile=$mosConfig_absolute_path."/seyretfiles/cache/pro/clipfishde/".$subdir.$md5cachefile;
	$fullmd5cachefilesite=$mosConfig_live_site."/seyretfiles/cache/pro/clipfishde/".$subdir.$md5cachefile;
...
if ($generatenewfile=="1"){
			...
			$dwnlink=clipfishdegeneratevideodownloadlink($vcode, $pro, "embed");
...
<image>".$vthumb."</image>
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/clipfishde";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
			...
			$makedir=$mosConfig_absolute_path."/seyretfiles/cache/pro/clipfishde/ad";
			if (!is_dir($makedir))
					{
					$oldumask=umask(0);
					mkdir ($makedir,0755);
...
$embedvideo="<object classid=\"clsid:d27cdb6e-ae6d-11cf-96b8-444553540000\" codebase=\"http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,0,0\" width=\"".$videowidth."\" height=\"".$videoheight."\" id=\"player\" align=\"middle\"><param name=\"allowScriptAccess\" value=\"sameDomain\" /><param name=\"movie\" value=\"http://www.clipfish.de/videoplayer.swf?as=0&videoid=".$vcode."%3D%3D&r=1\" /><param name=\"wmode\" value=\"transparent\"><param name=\"quality\" value=\"high\" /><param name=\"bgcolor\" value=\"#999999\" /><embed src=\"http://www.clipfish.de/videoplayer.swf?as=0&videoid=".$vcode."%3D%3D&r=1\" quality=\"high\" bgcolor=\"#999999\" width=\"".$videowidth."\" height=\"".$videoheight."\" name=\"player\" align=\"middle\" allowScriptAccess=\"sameDomain\" type=\"application/x-shockwave-flash\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\"></embed></object>";
}
return $embedvideo;
...
function clipfishdegeneratevideodownloadlink($vcode, $pro, $dltask){
